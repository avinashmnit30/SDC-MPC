...
size_t N = 10;
double dt = 0.1;   // Based on Latency
...
double v_ref = 100;   // Reference Velocity
size_t x_start = 0;
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (t = 1; t < N; t++) {
      ...
      // Update Equations:
      fg[1 + x_start + t] = x1 - (x0 + v0 * CppAD::cos(psi0) * dt);  // x[t+1] = x[t] + v[t] * cos(psi[t]) * dt
      fg[1 + y_start + t] = y1 - (y0 + v0 * CppAD::sin(psi0) * dt);  // y[t+1] = y[t] + v[t] * sin(psi[t]) * dt
      fg[1 + psi_start + t] = psi1 - (psi0 - v0 / Lf * delta0 * dt); // psi[t+1] = psi[t] + v[t] / Lf * delta[t] * dt
      fg[1 + v_start + t] = v1 - (v0 + a0 * dt);   // v[t+1] = v[t] + a[t] * dt
      fg[1 + cte_start + t] = cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi0) * dt));   // cte[t+1] = f(x[t]) - y[t] + v[t] * sin(epsi[t]) * dt
      fg[1 + epsi_start + t] = epsi1 - ((psi0 - psides0) - v0 / Lf * delta0 * dt);  // epsi[t+1] = psi[t] - psides[t] + v[t] * (delta[t] / Lf) * dt
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set to the max negative and positive values.
  ...
  // Acceleration Limits (-ve for decceleration)
